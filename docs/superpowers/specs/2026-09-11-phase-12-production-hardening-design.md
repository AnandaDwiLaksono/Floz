...
**Canonical owner budget (P0, one API process and one worker runtime).** Keep one API AuthService database owner with physical pool **max: 2**; keep **max: 1 for every Worker/runtime/temporary/maintenance owner** and **WORKER_CONCURRENCY=1 per BullMQ worker**. Existing Phase 0–11 concurrency contracts demonstrate that two concurrent API database lanes are required: Postgres.js max1 breaks established membership/team concurrency behavior. Max2 is the minimum evidenced capacity required; no repository evidence currently justifies max greater than 2. Keep actual owner boundaries; no pool-sharing refactor or second API owner is required. Reject P0 overrides increasing caps/concurrency or replicas without a revised aggregate budget. The present factory hardcodes max10 (`database/src/index.ts:7-9`); current concurrency defaults to 5 per worker (`packages/config/src/index.ts:17`). Numbers below are maximum application connection slots, not assertions of eagerly opened sockets or provider backend counts behind a pooler.

| Actual current owner / purpose | Current construction and lifetime | P0 count × max | Close owner |
|---|---|---|---|
| API AuthService: Better Auth and all borrowing API services; API PG readiness also borrows | One max10 client (`apps/api/src/auth.ts:14,37-39`) | 1 × 2 = 2 | AuthService, after API drain |
...
Adding any PostgreSQL pool owner requires revisiting and approving this aggregate budget. API persistent maximum is **2** from one AuthService owner; Worker persistent maximum is **4**, with **2** separately bounded transient runtime clients.

**Exact totals:** four persistent worker clients plus API = **6** persistent slots; one notification job client plus one notification/reconciliation client = **2** transient runtime slots; **normal runtime ceiling 8**. Explicit worker readiness adds **1**, so runtime plus readiness = **9**. Conservatively allow one readiness, one migration and one backup simultaneously with runtime: **maintenance ceiling 11** on the source database. API readiness adds **0** because it borrows the AuthService pool; Docker worker health adds **0**. Migration/backup invocations are serialized per purpose; no overlapping backups or runners beyond the counted owner. An isolated serial pg_restore uses **1 additional connection on a different target database**, never the source budget; integrity checks reuse a single sequential target connection after restore, not a parallel pool. Phase 13 Gate B must verify actual provider connection quota and operational headroom for ceiling 11 plus provider, administration and other-session usage.
...
Apply Section 5 connect, statement, lock and readiness bounds; configure statement_timeout/lock_timeout as server settings, not a JavaScript Promise race masquerading as query cancellation. PostgreSQL physical pool max is 2 for the one approved API AuthService owner and max1 for every Worker/runtime/temporary/maintenance owner; connect_timeout is 5 seconds, statement_timeout is 10 seconds server-side, and lock_timeout is 3 seconds server-side. Phase 12 MUST NOT implement an artificial client timeout that returns failure while an already-dispatched SQL mutation may still commit later. postgres.js pipelining is treated as part of the current driver semantics: max1 limits a Worker physical connection count, but it does not mean a cancellable application acquisition queue exists. Mutations are never automatically retried after an uncertain transport/cancellation outcome. API readiness has one outstanding PG probe per runtime, including queue wait; timed-out probes cannot accumulate. Worker readiness uses a private max1 client, separate from the reserved claim session. Cancel/terminate probe resources on deadline, suppress replacement until settled, and complete successful cleanup inside the overall 3-second readiness budget. A wrapper deadline alone is not query cancellation.
...
8. Worker close(false) graceful behavior, cached close(true) non-escalation, long job/loop termination, one API AuthService owner max2, all Worker owners max1, exact DB owners/caps/normal8/readiness9/maintenance11 totals and 30/35-second bounds. Heartbeat 15-second write, identity, age45, startup stale cleanup before healthy, stopping and hung-progress failures; Docker local-only interval30/start30/timeout5/retries3 with no PG/Redis activity.
9. Explicit worker readiness PG/Redis probes at 2 seconds each/3 seconds overall, single-flight/cleanup and deploy/operator/Phase13-only cadence; API PG probe borrowing with no Redis/new pool. Preserve TLS failure, Redis reconnect/quota behavior, queue attempts/backoff/retention exact values; diagnostic CLI cannot mutate/re-drive. After implementation authorization, require membership concurrency exactly [200,409], never 500; team concurrency exactly [200,200]; full @floz/api green; focused config/database/worker tests, lint and typecheck green.
...
**P1:** global/distributed rate limits before multiple API processes; basic metrics, centralized log shipping and measured pool/provider tuning; enhanced client error reporting. If Floz later requires a hard pre-dispatch admission deadline, evaluate an explicit cancellable application admission layer or a database abstraction/driver that can prove pre-execution cancellation semantics. Do NOT implement that admission layer in Phase 12 P0. Do NOT replace Postgres.js. P1 is not a Gate A or Gate B blocker.
...
Phase 12 design decisions are closed except for the API physical-capacity sub-contract reopened after regression evidence and approved here. Checkpoint A remains accepted for configuration, TLS, timeout and owner architecture. Checkpoint B implementation is preserved, but acceptance remains blocked until the revised API capacity is implemented and full API regression is green. Canonical contracts are the fixed values in Sections 4–18: one API AuthService owner with physical max2; Worker/runtime/temporary/maintenance owners max1; Worker concurrency 1; exact approved HTTPS origins and cookie topology are supplied as Gate B environment evidence; Caddy trust is the deployment-approved narrow source boundary; pilot objectives are RPO 24 hours and RTO 4 hours; worker heartbeat is 15 seconds; Docker local health is interval 30 seconds, start period 30 seconds, timeout 5 seconds, retries 3, with no DB/Redis calls; dependency readiness is explicit/deployment/Phase13-cadence only at 2 seconds each and 3 seconds overall; stale heartbeat cleanup precedes healthy state; combined API/worker normal runtime ceiling is 8, runtime-plus-readiness 9, maintenance 11; backup is custom pg_dump → age recipient → encrypted SHA-256 metadata → dedicated R2 prefix with 7 daily/4 weekly retention and separate identity.

Phase 13 Gate B must verify actual provider connection quota and operational headroom for source ceiling 11 plus provider/admin/other-session usage. Gate A remains deterministic engineering evidence with no production deploy. Gate B is actual-environment Phase 13 operational acceptance. Neither gate is passed by this document.
