...
export async function markOutboxDispatched(db: Db, input: { id: string; claimToken: string; now: Date }) {
  return (await db`UPDATE outbox_events SET status='DISPATCHED',dispatched_at=${input.now.toISOString()},claimed_by=NULL,claimed_until=NULL WHERE id=${input.id} AND claimed_by=${input.claimToken} AND claimed_until > ${input.now.toISOString()} AND status IN ('PENDING','FAILED') RETURNING id`).length > 0;
}

export async function markOutboxRetry(db: Db, input: { id: string; claimToken: string; availableAt: Date; now: Date }) {
  return (await db`UPDATE outbox_events SET status='FAILED',available_at=${input.availableAt.toISOString()},claimed_by=NULL,claimed_until=NULL WHERE id=${input.id} AND claimed_by=${input.claimToken} AND claimed_until > ${input.now.toISOString()} AND status IN ('PENDING','FAILED') RETURNING id`).length > 0;
}
