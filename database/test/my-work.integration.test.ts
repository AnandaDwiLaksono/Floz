...
describe.skipIf(!databaseUrl)('my work integration', () => {
  ...
  beforeAll(async () => {
    ...
    cancelledStatusId = statuses.find((status) => status.category === 'CANCELLED')!.id;
  ...
  it('projects assigned active tasks into timezone-aware, ordered buckets and counts', async () => {
    ...
    expect(result.today.map((task) => task.taskKey)).toEqual(['TODAY-A', 'TODAY-B', 'EQUAL']);
    ...
    expect(result.counts).toEqual({ today: 3, upcoming: 2, overdue: 3 });
  });
});
