...
export default function MemberDashboardPage() {
  ...
  const load = useCallback(async () => { setLoading(true); setError(''); try { await api.workspaces.get(workspaceId); setData((await api.workspaces.dashboardMember(workspaceId)).data); } catch (err) { setError(err instanceof ApiError && err.status === 403 ? 'You do not have permission to view this page.' : err instanceof Error ? err.message : 'Dashboard unavailable'); } finally { setLoading(false); } }, [workspaceId]);
