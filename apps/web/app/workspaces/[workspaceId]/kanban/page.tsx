...
export default function KanbanPage() {
  ...
  const move = async (card: KanbanCardSummary, statusId: string) => {
    ...
    if (!previous || !previous.columns.some((column) => column.status.id === statusId && column.status.is_active !== false)) return;
  ...
  return <div className="space-y-4">
    ...
    <div className="flex gap-4 overflow-x-auto pb-4">{board?.columns.map((column) => { const archived = column.status.is_active === false; return <section key={column.status.id} aria-label={archived ? `${column.status.name} (archived, drop unavailable)` : column.status.name} onDragOver={archived ? undefined : (e) => e.preventDefault()} onDrop={archived ? undefined : () => dragged && void move(dragged, column.status.id)} className={`w-72 shrink-0 rounded border bg-gray-50 p-3 dark:bg-gray-900 ${archived ? 'border-dashed opacity-75' : ''}`}><h3 className="flex justify-between font-semibold"><span>{column.status.name}</span><span>{column.task_count}</span></h3>{archived && <p className="mt-2 text-xs text-gray-500">Drop unavailable</p>}<div className="mt-3 space-y-3">{column.cards.map((card) => <article key={card.id} draggable onDragStart={() => setDragged(card)} className="rounded border bg-white p-3 shadow-sm dark:bg-gray-800"><button onClick={() => router.push(taskRoute(workspaceId, card.id))} className="block w-full text-left"><span className="text-xs font-bold text-blue-600">{card.task_key}</span><strong className="block">{card.title}</strong></button><p className="mt-2 text-xs">{card.priority} · {card.assignees.find((item) => item.is_primary)?.full_name || 'Unassigned'}</p><p className={card.is_overdue ? 'text-xs text-red-600' : 'text-xs text-gray-500'}>{card.is_overdue ? 'Overdue · ' : ''}{due(card)}</p><select aria-label={`Change status for ${card.title}`} className="mt-2 w-full text-xs" value={card.status.id} onChange={(e) => void move(card, e.target.value)}><option value={card.status.id}>{column.status.name}</option>{board.columns.filter((target) => target.status.id !== card.status.id && target.status.is_active !== false).map((target) => <option key={target.status.id} value={target.status.id}>{target.status.name}</option>)}</select></article>)}</div></section>; })}</div>
  </div>;
}
