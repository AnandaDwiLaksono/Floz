...
export class FlozService {
  ...
  async updateTeam(id: string, input: { name?: string; description?: string | null; managerUserId?: string | null }): Promise<TeamRow> { return (await this.sql<TeamRow[]>`UPDATE teams SET name = COALESCE(${input.name ?? null}, name), description = COALESCE(${input.description ?? null}, description), manager_user_id = CASE WHEN ${input.managerUserId === undefined} THEN manager_user_id ELSE ${input.managerUserId ?? null} END, updated_at = NOW() WHERE id = ${id} RETURNING id, workspace_id, name, description, manager_user_id, is_active AS "isActive", created_at, updated_at`)[0]; }
