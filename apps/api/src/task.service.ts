...
export interface CalendarTaskSummary { id: string; task_key: string; title: string; priority: Priority; start_at: string | null; due_at: string | null; is_deadline_only: boolean; status: { id: string; name: string; code: string; category: string; is_active: boolean }; primary_assignee: { id: string; full_name: string } | null; }
type TaskRow = { id: string; workspace_id: string; task_key: string; title: string; description: string | null; workflow_id: string; status_id: string; priority: Priority; team_id: string | null; creator_id: string; start_at: Date | null; due_at: Date | null; completed_at: Date | null; version: number; created_at: Date; updated_at: Date; status_code: string; status_name: string; status_category: string; status_is_active: boolean };
...
type CalendarRow = { id: string; task_key: string; title: string; priority: Priority; start_at: Date | string | null; due_at: Date | string | null; status_id: string; status_code: string; status_name: string; status_category: string; status_is_active: boolean; primary_assignee_id: string | null; primary_assignee_name: string | null; is_deadline_only: boolean };
...
function parseTaskIsoTimestamp(value: string): Date | null { const match = isoTimestamp.exec(value); if (!match) return null; const year = Number(match[1]); const month = Number(match[2]); const day = Number(match[3]); const calendar = new Date(Date.UTC(year, month - 1, day)); if (calendar.getUTCFullYear() !== year || calendar.getUTCMonth() !== month - 1 || calendar.getUTCDate() !== day) return null; const result = new Date(value); return Number.isFinite(result.getTime()) ? result : null; }
...
export class TaskService {
  constructor(@Inject(AuthService) private readonly authService: AuthService, @Inject(ReportingClock) private readonly clock: ReportingClock) {}
  ...
  private task(row: TaskRow, assignees: AssigneeRow[]) { const { status_code, status_name, status_category, status_is_active, ...task } = row; return { ...task, status: { id: row.status_id, code: status_code, name: status_name, category: status_category, is_active: status_is_active }, assignees }; }
  ...
  private validateCreate(input: CreateTaskDto) { const primaryCount = input.assignees?.filter((a) => a.is_primary).length ?? 0; if (!input.title?.trim() || input.title.trim().length > 255 || (input.priority && !priorities.includes(input.priority)) || primaryCount > 1) throw new BadRequestException('VALIDATION_ERROR'); if (input.start_at && input.due_at && new Date(input.start_at) > new Date(input.due_at)) throw new BadRequestException('VALIDATION_ERROR'); }
  private iso(value: Date | string | null) { return value === null ? null : new Date(value).toISOString(); }
  private parseIsoTimestamp(value: string | undefined) { if (!value) return null; const parsed = parseTaskIsoTimestamp(value); if (!parsed) throw new BadRequestException('VALIDATION_ERROR'); return parsed.toISOString(); }
  private async validateTeamScope(workspaceId: string, teamId: string | undefined) { if (!teamId) return; if (!uuidPattern.test(teamId)) throw new BadRequestException('VALIDATION_ERROR'); if (!(await this.sql`SELECT id FROM teams WHERE id=${teamId} AND workspace_id=${workspaceId}`)[0]) throw new BadRequestException('TEAM_SCOPE_MISMATCH'); }
  private async validateAssigneeScope(workspaceId: string, assigneeId: string | undefined) { if (!assigneeId) return; if (!uuidPattern.test(assigneeId)) throw new BadRequestException('VALIDATION_ERROR'); if (!(await this.sql`SELECT user_id FROM workspace_memberships WHERE workspace_id=${workspaceId} AND user_id=${assigneeId} AND status='ACTIVE'`)[0]) throw new BadRequestException('CROSS_WORKSPACE_REFERENCE'); }
  private async validateStatusScope(workspaceId: string, statusId: string | undefined) { if (!statusId) return; if (!uuidPattern.test(statusId)) throw new BadRequestException('VALIDATION_ERROR'); if (!(await this.sql`SELECT s.id FROM task_statuses s JOIN workflows w ON w.id=s.workflow_id WHERE s.id=${statusId} AND w.workspace_id=${workspaceId}`)[0]) throw new BadRequestException('STATUS_SCOPE_MISMATCH'); }
  private async detailSql(sql: SqlClient, workspaceId: string, id: string) { const row = (await sql<TaskRow[]>`SELECT t.*, s.code AS status_code, s.name AS status_name, s.category AS status_category, s.is_active AS status_is_active FROM tasks t JOIN task_statuses s ON s.id=t.status_id WHERE t.id=${id} AND t.workspace_id=${workspaceId} AND t.deleted_at IS NULL`)[0]; if (!row) throw new NotFoundException('NOT_FOUND'); const assignees = await sql<AssigneeRow[]>`SELECT ta.user_id, ta.is_primary, u.name AS full_name FROM task_assignees ta JOIN users u ON u.id=ta.user_id WHERE ta.task_id=${id} ORDER BY ta.is_primary DESC, u.name`; return this.task(row, assignees); }
  ...
  async list(workspaceId: string, query: TaskQueryDto) {
    ...
    const dueTo = this.parseIsoTimestamp(query.due_to);
    if (dueFrom && dueTo && dueFrom >= dueTo) throw new BadRequestException('VALIDATION_ERROR');
    await Promise.all([this.validateTeamScope(workspaceId, query.team_id), this.validateAssigneeScope(workspaceId, query.assignee_id), this.validateStatusScope(workspaceId, query.status_id)]);
    ...
    const priorityRank = this.sql`CASE t.priority WHEN 'URGENT' THEN 1 WHEN 'HIGH' THEN 2 WHEN 'MEDIUM' THEN 3 WHEN 'LOW' THEN 4 ELSE 5 END`;
    const cursorPriorityRank = cursor?.value ? ({ URGENT: 1, HIGH: 2, MEDIUM: 3, LOW: 4 } as const)[cursor.value as Priority] ?? 5 : null;
    const after = !cursor ? this.sql`` : sortName === 'priority' ? direction === 'ASC' ? this.sql` AND (${priorityRank},t.id) > (${cursorPriorityRank},${cursor.id})` : this.sql` AND (${priorityRank},t.id) < (${cursorPriorityRank},${cursor.id})` : sortName !== 'due_at' ? direction === 'ASC' ? this.sql` AND (${this.sql.unsafe(column)},t.id) > (${cursor.value},${cursor.id})` : this.sql` AND (${this.sql.unsafe(column)},t.id) < (${cursor.value},${cursor.id})` : direction === 'ASC' ? cursor.value === null ? this.sql` AND t.due_at IS NULL AND t.id > ${cursor.id}` : this.sql` AND (t.due_at > ${cursor.value} OR (t.due_at = ${cursor.value} AND t.id > ${cursor.id}) OR t.due_at IS NULL)` : cursor.value === null ? this.sql` AND (t.due_at IS NOT NULL OR (t.due_at IS NULL AND t.id < ${cursor.id}))` : this.sql` AND (t.due_at < ${cursor.value} OR (t.due_at = ${cursor.value} AND t.id < ${cursor.id}))`;
    const nulls = sortName === 'due_at' ? this.sql.unsafe(direction === 'ASC' ? ' NULLS LAST' : ' NULLS FIRST') : this.sql``;
    const prioritiesFilter = query.priority ? query.priority.split(',').filter((value): value is Priority => priorities.includes(value as Priority)) : [];
    if (query.priority && prioritiesFilter.length !== query.priority.split(',').length) throw new BadRequestException('VALIDATION_ERROR');
    if (query.bucket && !['active', 'completed'].includes(query.bucket)) throw new BadRequestException('VALIDATION_ERROR');
    if (query.overdue !== undefined && query.overdue !== 'true') throw new BadRequestException('VALIDATION_ERROR');
    const evaluationAt = query.overdue === 'true' ? this.clock.now() : null;
    const rows = await this.sql<TaskRow[]>`SELECT t.*,s.code AS status_code,s.name AS status_name,s.category AS status_category,s.is_active AS status_is_active FROM tasks t JOIN task_statuses s ON s.id=t.status_id WHERE t.workspace_id=${workspaceId} AND t.deleted_at IS NULL ${query.bucket === 'active' ? this.sql` AND s.is_terminal = false AND s.category <> 'CANCELLED'` : query.bucket === 'completed' ? this.sql` AND t.completed_at IS NOT NULL` : this.sql``}${evaluationAt ? this.sql` AND s.is_terminal = false AND s.category <> 'CANCELLED' AND t.due_at < ${evaluationAt.toISOString()}` : this.sql``}${query.q ? this.sql` AND (t.title ILIKE ${`%${query.q}%`} OR t.task_key ILIKE ${`%${query.q}%`})` : this.sql``}${query.status_id ? this.sql` AND t.status_id=${query.status_id}` : this.sql``}${prioritiesFilter.length ? this.sql` AND t.priority IN ${this.sql(prioritiesFilter)}` : this.sql``}${query.team_id ? this.sql` AND t.team_id=${query.team_id}` : this.sql``}${query.assignee_id ? this.sql` AND EXISTS (SELECT 1 FROM task_assignees ta WHERE ta.task_id=t.id AND ta.user_id=${query.assignee_id})` : this.sql``}${dueFrom ? this.sql` AND t.due_at >= ${dueFrom}` : this.sql``}${dueTo ? this.sql` AND t.due_at < ${dueTo}` : this.sql``}${after} ORDER BY ${sortName === 'priority' ? this.sql`CASE t.priority WHEN 'URGENT' THEN 1 WHEN 'HIGH' THEN 2 WHEN 'MEDIUM' THEN 3 WHEN 'LOW' THEN 4 ELSE 5 END` : this.sql.unsafe(column)} ${this.sql.unsafe(direction)}${nulls},t.id ${this.sql.unsafe(direction)} LIMIT ${limit + 1}`;
  ...
  async update(workspaceId: string, actorId: string, role: TaskRole, id: string, input: UpdateTaskDto) { this.mutation(role, 'mutate'); if (!this.validVersion(input.version) || (input.priority && !priorities.includes(input.priority)) || (input.title !== undefined && !input.title.trim())) throw new BadRequestException('VALIDATION_ERROR'); if (input.start_at && input.due_at && new Date(input.start_at) > new Date(input.due_at)) throw new BadRequestException('VALIDATION_ERROR'); return this.authService.database.sql.begin(async (sql: TransactionSql) => { await patchTaskRecordTx(sql, workspaceId, actorId, id, { ...input, version: input.version! }); return this.detailSql(sql, workspaceId, id); }); }
