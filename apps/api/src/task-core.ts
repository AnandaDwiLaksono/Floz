...
export type TaskCreationInput = { title?: string; description?: string | null; priority?: string; workflow_id?: string; status_id?: string; team_id?: string | null; start_at?: string | null; due_at?: string | null };
...
export async function validateTaskTemplateReferences(sql: TransactionSql, workspaceId: string, input: TaskCreationInput): Promise<TaskTemplateReferences> {
  ...
  if (!workflow) throw new BadRequestException('WORKFLOW_SCOPE_MISMATCH');
  if (input.status_id && !(await sql`SELECT id FROM task_statuses WHERE id=${input.status_id} AND workflow_id=${workflow.workflow_id}`)[0]) throw new BadRequestException('STATUS_SCOPE_MISMATCH');
...
export async function createTaskRecordTx(sql: TransactionSql, workspaceId: string, actorId: string, input: TaskCreationInput, workflow: TaskTemplateReferences) {
  ...
  const task = (await sql<{ id: string }[]>`INSERT INTO tasks(workspace_id,task_key,title,description,workflow_id,status_id,priority,team_id,creator_id,start_at,due_at) VALUES(${workspaceId},${key.key},${input.title!.trim()},${input.description ?? null},${workflow.workflow_id},${input.status_id ?? workflow.status_id},${input.priority ?? 'MEDIUM'},${input.team_id ?? null},${actorId},${input.start_at ?? null},${input.due_at ?? null}) RETURNING id`)[0];
...
export async function createTaskAssigneesTx(sql: TransactionSql, workspaceId: string, taskId: string, actorId: string, assignees: TaskAssigneeInput[]) {
  if (assignees.filter((a) => a.is_primary).length > 1) throw new BadRequestException('VALIDATION_ERROR');
  for (const assignee of assignees) if (!(await sql<{ user_id: string }[]>`SELECT user_id FROM workspace_memberships WHERE workspace_id=${workspaceId} AND user_id=${assignee.user_id} AND status='ACTIVE'`)[0]) throw new BadRequestException('CROSS_WORKSPACE_REFERENCE');
...
export async function writeTaskHistoryTx(sql: TransactionSql, taskId: string, actorId: string) {
  await sql`INSERT INTO task_history(task_id,actor_user_id,event_type,metadata) VALUES(${taskId},${actorId},'CREATED',${JSON.stringify({})}::jsonb)`;
}
