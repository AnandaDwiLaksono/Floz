...
export class RecurrenceService {
  ...
  async stop(workspaceId: string, id: string) { const updated = (await this.sql<{ id: string }[]>`UPDATE recurrence_rules SET is_active=false,next_run_at=NULL,updated_at=NOW() WHERE id=${id} AND workspace_id=${workspaceId} RETURNING id`)[0]; if (!updated) throw new NotFoundException('NOT_FOUND'); return this.get(workspaceId, id); }
}
