...
type TaskRow = { id: string; workspace_id: string; task_key: string; title: string; description: string | null; workflow_id: string; status_id: string; priority: string; team_id: string | null; creator_id: string; start_at: Date | null; due_at: Date | null; completed_at: Date | null; version: number; recurrence_rule_id: string | null; created_at: Date; updated_at: Date; status_code: string; status_name: string; status_category: string; status_is_active: boolean };
...
function task(row: TaskRow, assignees: { user_id: string; is_primary: boolean; full_name: string }[]) { const { status_code, status_name, status_category, status_is_active, ...rest } = row; return { ...rest, start_at: iso(row.start_at), due_at: iso(row.due_at), completed_at: iso(row.completed_at), created_at: iso(row.created_at), updated_at: iso(row.updated_at), status: { id: row.status_id, code: status_code, name: status_name, category: status_category, is_active: status_is_active }, assignees }; }
